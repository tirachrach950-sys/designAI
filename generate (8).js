export default async function handler(req, res) {
  ...
  try {
    ...
    if (pexelsQuery) {
      ...
      // Fallback to Unsplash if Pexels failed or no key
      if (!imageUrl) {
        const unsplashQuery = encodeURIComponent(pexelsQuery.split(' ').slice(0, 3).join(','));
        imageUrl = `https://source.unsplash.com/1600x900/?${unsplashQuery}`;
        imagePhotographer = 'Unsplash';
      }
    }
